...
function submitNavigationForm(form, url, is_ajax) {
    ...
    if ((parseFloat($(from).value) > parseFloat($(to).value)) || ((parseFloat($(from).value) == 0 && parseFloat($(to).value) == 0))) {
    ...
    if (!is_ajax) {
        ...
        var request = new Ajax.Request(url, {
            ...
            onSuccess: function(transport) {
                ...
                replaceBanner('content-header', response.banner);
                try {
                    if (response.eval_js) {
...
function replaceLeftRightNavigationBlocks(element, content)
{
 ...
 function replaceBanner(element, content) {
     var element = $(element);
    if (content && element) {
        if (content && content.toElement) {
            content = content.toElement();
            ...
            var tempElement = document.createElement('div');
            tempElement.innerHTML = content;
            tempElement.setAttribute("id", element.readAttribute('id'));
            content = tempElement;
            content.hide();
        }
        ...
        Effect.BlindUp(element, { afterFinish: function () { element.innerHTML = ''; } });
    }
 }

function initSlider(code, min, max, curr_min, curr_max, url, is_ajax, unit) {
    ...
    s1.options.onChange = function(value) {
        ...
        if (value[0] >= 0 && value[1] >= 0) {
            ...
            $(code + '-value-from').innerHTML = parseInt(1 * value[0]) + ' ' + unit;
            $(code + '-value-to').innerHTML = parseInt(1 * value[1]) + ' ' + unit;
    ...
    slider_data.from = parseInt(s1.values[0]) + ' ' + unit;
    slider_data.to = parseInt(s1.values[1]) + ' ' + unit;
