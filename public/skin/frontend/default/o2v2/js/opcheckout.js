            ...
            var request = new Ajax.Request(
                this.saveUrl,
                {
                    ...
                    onSuccess: function(transport){
                        jQuery('#checkout-step-opc-billing').hide();
                        var response = transport.responseText.evalJSON(true);
                        if(response.account_popin && response.headMenu && response.accountMenu)
                        {
                            $$(response.headMenu+' '+ response.accountMenu)[0].remove();
                            $$(response.headMenu)[0].insert(response.account_popin);
                        }
                        self.onSave(transport);
                        location.reload();
                    },
...
//        $('billing:use_for_shipping_yes').checked = flag;
    ...
    syncWithBilling: function () {
        ...
        if (!$('billing-address-select') || !$('billing-address-select').value) {
            ...
            for (var elemIndex in arrElements) {
                if (arrElements[elemIndex].id) {
                	console.log("syncWithBilling : " + arrElements[elemIndex].id);
                    var sourceField = $(arrElements[elemIndex].id.replace(/^shipping:/, 'billing:'));
                    if (sourceField){
                    	console.log("syncWithBilling : " + arrElements[elemIndex].id + " Val : " + sourceField.value);
...
ShippingMethod.prototype = {
    ...
    validate: function() {
        ...
        jQuery.fn.microModal.dialogs['#confirmPopinError'].open();
    	jQuery("#contactMsg").html(Translator.translate('Please specify shipping method.'));
        return false;
    },

    save: function(hasOrdonnance){
        hasOrdonnance = (!hasOrdonnance) ? hasOrdonnance : true;
        if (checkout.loadWaiting!=false) return;
        if (this.validate()) {
...
//ordonnance
var Ordonnance = Class.create();
Ordonnance.prototype = {
    initialize: function(form, saveUrl){
        this.form = form;
        if ($(this.form)) {
            $(this.form).observe('submit', function(event){this.save();Event.stop(event);}.bind(this));
    ...
    nextStep: function(transport){
        ...
        if (response.error) {
            //alert(response.message);
        	jQuery.fn.microModal.dialogs['#confirmPopinError'].open();
        	jQuery("#contactMsg").html(response.message);
            return false;
        }
